...
class JobDeleteViewTestCase(TestCase):
    ...
    def test_delete_data(self):
        """Tests that a POST request delete instance"""
        self.client.force_login(self.user)
        response = self.client.post(reverse('job_delete', kwargs={'company': self.company.slug, 'slug': self.job.slug}), follow=True)
        self.assertEqual(Job.objects.count(), 0)


    def test_success_url(self):
        """Tests that after successfull delete redirect on main page"""
        self.client.force_login(self.user)
        response = self.client.post(reverse('job_delete', kwargs={'company': self.company.slug, 'slug': self.job.slug}), follow=True)
